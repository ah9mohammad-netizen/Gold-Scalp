#!/usr/bin/env python3
"""Nested walk-forward sensitivity study for a layered XAU decision tree.
...
one candidate using only the preceding up-to-three-year history.  Selection
requires at least 20 trades, positive net PnL, PF >= 1.10, and no adequately
sampled chronological third with PF below 0.80.  Otherwise its learned action
...
Example:
    python scripts/research_adaptive_regime_tree.py /tmp/xau_canonical_repaired.csv --gap-guard
"""
...
def candidate_bank(count_per_context: int) -> List[TreeConfig]:
    """Create a bounded fractional-factorial sensitivity bank.

    Every factor level appears near-equally often per context.  Independent
    deterministic shuffles prevent a single hand-picked correlated sequence
    from silently dictating all parameter interactions.
    """
    if count_per_context < 6:
        raise ValueError("At least six candidates per context are needed to cover the declared levels")
    configs: List[TreeConfig] = []
    for context_index, (context, setup, direction) in enumerate(CONTEXTS):
        seed_base = 10_000 + context_index * 100
        levels = {
            "trend_adx_min": balanced_levels((18.0, 22.0, 26.0), count_per_context, seed_base + 1),
            "range_adx_max": balanced_levels((14.0, 18.0, 22.0), count_per_context, seed_base + 2),
            "trend_sep": balanced_levels((0.05, 0.15, 0.30), count_per_context, seed_base + 3),
            "range_sep": balanced_levels((0.10, 0.25, 0.40), count_per_context, seed_base + 4),
            "atr_min": balanced_levels((0.45, 0.65, 0.85), count_per_context, seed_base + 5),
            "atr_max": balanced_levels((1.50, 1.80, 2.10), count_per_context, seed_base + 6),
            "touch": balanced_levels((0.00, 0.20, 0.40), count_per_context, seed_base + 7),
            "rsi_level": balanced_levels((40.0, 45.0, 48.0) if setup == "pullback" else (25.0, 30.0, 35.0) if setup == "reversion" else (55.0, 60.0, 65.0), count_per_context, seed_base + 8),
            "rsi_reclaim": balanced_levels((30.0, 35.0, 40.0) if setup == "reversion" else (48.0, 50.0, 52.0), count_per_context, seed_base + 9),
            "cci": balanced_levels((0.0, 25.0, 50.0) if setup == "pullback" else (75.0, 100.0, 125.0), count_per_context, seed_base + 10),
            "bb": balanced_levels((0.15, 0.25, 0.35), count_per_context, seed_base + 11),
            "range_atr": balanced_levels((0.80, 1.00, 1.20), count_per_context, seed_base + 12),
            "require_vwap": balanced_levels((False, True), count_per_context, seed_base + 13),
            "vwap_distance": balanced_levels((0.00, 0.25, 0.50), count_per_context, seed_base + 14),
            "confirm": balanced_levels((False, True), count_per_context, seed_base + 15),
            "stop": balanced_levels((1.25, 1.50, 2.00), count_per_context, seed_base + 16),
            "rr": balanced_levels((1.25, 1.50, 2.00), count_per_context, seed_base + 17),
            "hold": balanced_levels((24, 48, 72), count_per_context, seed_base + 18),
        }
        for index in range(count_per_context):
            configs.append(
                TreeConfig(
                    config_id=f"{context}-C{index + 1:02d}",
                    context=context,
                    setup=setup,
                    direction=direction,
...
def pullback_signal(candidate: TreeConfig, current: Feature, previous: Feature) -> Optional[Signal]:
    ...
    if direction == "LONG":
        ...
        triggered = (
            touched and bar.close > current.ema50 and bar.close > bar.open
            and previous.rsi <= candidate.rsi_level and current.rsi >= candidate.rsi_reclaim
            and previous.cci <= candidate.cci_level < current.cci
        ...
        triggered = (
            touched and bar.close < current.ema50 and bar.close < bar.open
            and previous.rsi >= 100.0 - candidate.rsi_level and current.rsi <= 100.0 - candidate.rsi_reclaim
            and previous.cci >= -candidate.cci_level > current.cci
            and current.macd_hist < previous.macd_hist
        )
        if candidate.require_confirmation_break:
...
def breakout_signal(candidate: TreeConfig, current: Feature, previous: Feature) -> Optional[Signal]:
    ...
    if direction == "LONG":
        triggered = (
            ...
            and current.rsi >= candidate.rsi_level and current.cci >= candidate.cci_level
        )
    else:
        triggered = (
            ...
            and bar.high - bar.low >= candidate.breakout_range_atr * current.atr
            and current.macd_hist < 0.0 <= previous.macd_hist
            and current.rsi <= 100.0 - candidate.rsi_level and current.cci <= -candidate.cci_level
        )
    return Signal(candidate, direction) if triggered and vwap_ok(current, direction, candidate) else None


def reversion_signal(candidate: TreeConfig, current: Feature, previous: Feature) -> Optional[Signal]:
    assert current.rsi is not None and current.cci is not None and current.bb_upper is not None and current.bb_lower is not None
    assert previous.rsi is not None and previous.cci is not None and previous.bb_upper is not None and previous.bb_lower is not None
    bar, prior = current.bar, previous.bar
    # Candidate rsi_level here means the prior extreme.  rsi_reclaim is the
    # closed-candle recovery threshold, shared symmetrically.
    long_trigger = (
        prior.close <= previous.bb_lower and bar.low <= current.bb_lower and bar.close > current.bb_lower and bar.close > bar.open
        and previous.rsi <= candidate.rsi_level and current.rsi >= candidate.rsi_reclaim
        and previous.cci <= -candidate.cci_level < current.cci
    )
    short_trigger = (
        prior.close >= previous.bb_upper and bar.high >= current.bb_upper and bar.close < current.bb_upper and bar.close < bar.open
        and previous.rsi >= 100.0 - candidate.rsi_level and current.rsi <= 100.0 - candidate.rsi_reclaim
        and previous.cci >= candidate.cci_level > current.cci
    )
    if long_trigger and vwap_ok(current, "LONG", candidate):
        return Signal(candidate, "LONG")
    if short_trigger and vwap_ok(current, "SHORT", candidate):
...
def assessment(config: TreeConfig, fold: str, train: Simulation) -> CandidateAssessment:
    """Score a candidate using complete training history and chronological stability."""
    segment_width = (train.end - train.start) / 3
    ...
    for index in range(3):
        ...
        # A short segment cannot establish stability and is ignored rather than
        # falsely labelled good or bad. Full-window minimum count still applies.
        if len(sample) >= 3:
    ...
    eligible = (
        ...
        and full_pf >= 1.10
        and (worst is None or worst >= 0.80)
    )
    # Robust score privileges the weak sampled segment instead of just picking
    # the largest in-sample PF. A small count term only breaks exact ties.
    score = min(full_pf, worst) + len(train.trades) / 1_000_000.0 if eligible and worst is not None else full_pf + len(train.trades) / 1_000_000.0 if eligible and full_pf is not None else None
...
def rolling_folds(last_decision: datetime) -> List[Tuple[str, datetime, datetime, datetime, datetime]]:
    """January/July outer folds, each trained on no more than prior three years."""
    ...
    while test_start <= last_decision:
        ...
        train_start = max(RESEARCH_START, test_start.replace(year=test_start.year - 3))
...
def sensitivity_overview(assessments: Sequence[CandidateAssessment], selections: Sequence[Selection]) -> List[str]:
    ...
    for context, _, _ in CONTEXTS:
        ...
        lines.append(
            ...
            f"{sum(item.net_pnl > 0.0 for item in items)} | {best_pf} | "
...
def make_report(
    ...
    lines.extend(["", "## Sensitivity eligibility overview", "", *sensitivity_overview(assessments, selections)])
    lines.extend(
        [
            "",
...
def main() -> None:
    ...
    parser.add_argument("--report", default="XAU_ADAPTIVE_REGIME_TREE.md")
    parser.add_argument("--configs-csv", default="XAU_ADAPTIVE_REGIME_TREE_CONFIGS.csv")
    parser.add_argument("--sensitivity-csv", default="XAU_ADAPTIVE_REGIME_TREE_SENSITIVITY.csv")
    parser.add_argument("--selections-csv", default="XAU_ADAPTIVE_REGIME_TREE_SELECTIONS.csv")
    parser.add_argument("--policy-trades-csv", default="XAU_ADAPTIVE_REGIME_TREE_TRADES.csv")
    args = parser.parse_args()
    if args.candidates_per_context < 6:
        parser.error("--candidates-per-context must be at least 6")
    ...
    for fold, train_start, train_end, test_start, test_end in folds:
        ...
        for context, candidates in sorted(config_by_context.items()):
            ...
            for candidate in candidates:
                ...
                item = assessment(candidate, fold, train)
